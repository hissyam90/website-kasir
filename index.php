...
$h1 = mysqli_query($con,"select * from pesanan");
...
<html lang="en">
                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="row">
                            <div class="col-xl-3 col-md-6">
                                <div class="card bg-warning text-white mb-4">
                                    <div class="card-body">[<i class="fa fa-briefcase"></i>] Jumlah Pesanan: <?=$h2;?></div>
                                    </div>
                                </div>
                            </div>     
                                ...
                                <table id="datatablesSimple">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Jumlah Produk</th>
                                            <th>Status <i class="fa fa-cog"></i></th>
                                            <th>Aksi</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    ...
                                    //menghitung jumlah
                                    $hitungjumlah = mysqli_query($con,"select * from detailpesanan where idpesanan='$idorder'");
                                    $jumlah = mysqli_num_rows($hitungjumlah);

                                    // ubah warna sesuai status
                                    $statusColor = '';
                                    if ($status == 'Pending') {
                                        $statusColor = 'badge bg-warning'; // Yellow
                                    } elseif ($status == 'Processed') {
                                        $statusColor = 'badge bg-info'; // Blue
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <button type="button" class="btn btn-info text-white" data-bs-toggle="modal" data-bs-target="#confirmStatusChange<?=$idorder;?>">
                                <i class="fa fa-cog"></i>
                                </button>
    ...
    <div class="modal fade" id="confirmStatusChange<?=$idorder;?>" tabindex="-1" aria-labelledby="confirmStatusChangeLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="confirmStatusChangeLabel">Konfirmasi Perubahan Status</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="post" action="function.php">
                    <div class="modal-body">
                        <?php if ($status == 'Pending'): ?>
                            Apakah Anda ingin mengubah status pesanan ini dari <strong><?=$status;?></strong> menjadi <strong>Processed</strong>?
                            <input type="hidden" name="newstatus" value="Processed">
                        <?php elseif ($status == 'Processed'): ?>
                            Anda ingin mengubah status pesanan ini dari <strong><?=$status;?></strong> menjadi <strong>Accepted</strong> atau kembali ke <strong>Pending</strong>?
                            <input type="hidden" name="newstatus" value="Accepted">
                            <input type="hidden" name="returnstatus" value="Pending">
                        <?php elseif ($status == 'Accepted'): ?>
                            Apakah Anda ingin mengubah status pesanan ini dari <strong><?=$status;?></strong> kembali menjadi <strong>Processed</strong>?
                            <input type="hidden" name="newstatus" value="Processed">
                            <input type="hidden" name="returnstatus" value="Pending">
                        <?php endif; ?>
                    ...
                    <div class="modal-footer">
                        <?php if ($status == 'Accepted'): ?>
                            <button type="submit" name="returnstatus" value="Processed" class="btn btn-info text-white">Return to Processed</button>
                        <?php elseif ($status == 'Processed'): ?>
                            <button type="submit" name="returnstatus" value="Pending" class="btn btn-warning text-white">Return to Pending</button>
                            <button type="submit" name="change_status" value="Accepted" class="btn btn-success text-white">Ubah ke Accepted</button>
                        <?php elseif ($status == 'Pending'): ?>
                            <button type="submit" name="change_status" value="Processed" class="btn btn-info text-white">Ubah ke Processed</button>
                        <?php endif; ?>
